...
fn main() {
    ...
    let builder = gtk::Builder::from_resource("/dev/lex/Spotdora/window.ui");
    ...
    if cfg!(debug_assertions) {
        ...
        gtk_app.set_resource_base_path(Some("/dev/lex/Spotdora"));
    }
...
fn setup_gtk(settings: &settings::SpotSettings) {
    ...
    let res = gio::Resource::load(config::PKGDATADIR.to_owned() + "/spotdora.gresource")
        .expect("Could not load resources");
    gio::resources_register(&res);

    let provider = gtk::CssProvider::new();
    provider.load_from_resource("/dev/lex/Spotdora/app.css");
...
fn setup_credits(about: libadwaita::AboutDialog) {
    // Read from AUTHORS at compile time and update the about dialog
