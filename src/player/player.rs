    ...
    fn create_player(&mut self, session: Session) -> Arc<Player> {
        ...
        Player::new(player_config, session, soft_volume, move || match backend {
            AudioBackend::GStreamer(pipeline) => {
                let backend = audio_backend::find(Some("gstreamer".to_string()))
                    .or_else(|| audio_backend::find(Some("pulseaudio".to_string())))
                    .unwrap();
                backend(Some(pipeline), AudioFormat::default())
            }
            AudioBackend::PulseAudio => {
                info!("using pulseaudio (PipeWire integration)");
                env::set_var("PULSE_PROP_application.name", "Spotdora");
                env::set_var("PULSE_PROP_application.icon_name", "dev.lex.Spotdora");
                env::set_var("PULSE_PROP_application.id", "dev.lex.Spotdora");
                env::set_var("PULSE_PROP_media.role", "music");
                env::set_var("PULSE_PROP_media.category", "playback");
                env::set_var("PULSE_PROP_media.software", "Spotdora");
